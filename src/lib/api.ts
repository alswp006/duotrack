...
export type PostGenerateProblemsResult =
  | { ok: true; data: GenerateProblemSetResponse; code?: undefined }
  | { ok: false; code: DiagnoseErrorCode; data?: undefined };
...
export async function postGenerateProblems(
  req: GenerateProblemSetRequest
): Promise<PostGenerateProblemsResult> {
  try {
    const base = import.meta.env.VITE_AI_API_BASE ?? "";
    const headers: Record<string, string> = { "Content-Type": "application/json" };
    const sessionId = getSessionId();
    ...
    const response = await fetch(`${base}/generate-problems`, {
      method: "POST",
      headers,
      body: JSON.stringify(req),
    });
    ...
    if (response.ok) {
      const data = (await response.json()) as GenerateProblemSetResponse;
      return { ok: true, data };
    }
